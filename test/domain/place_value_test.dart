...
/// Draws a good number of tasks from a place-value lesson.
List<Task> _draw(String id, {int runs = 60, int count = 20}) => [
      for (var seed = 0; seed < runs; seed++)
        ...generateTasks(lesson: lessonById(id), count: count, seed: seed)
    ];
...
void main() {
  ...
  for (final MapEntry(key: id, value: biggest) in ranges.entries) {
    group(id, () {
      ...
      test('every answer stays inside the range of its group', () {
        for (final task in tasks) {
          expect(task.expected, lessThanOrEqualTo(biggest), reason: task.prefix);
        }
      });

      test('no place above the ones this lesson drills is named', () {
        for (final task in tasks) {
          for (var place = highest + 1; place < 4; place++) {
            expect(task.places[place], 0, reason: task.prefix);
          }
        }
      });
      ...
      test('the top place is capped at nine, so the range holds', () {
        for (final task in tasks) {
          expect(task.places[highest], lessThanOrEqualTo(9),
              reason: task.prefix);
        }
      });
    });
  ...
  test('the text names the parts from the ones upwards', () {
    const task =
        Task(a: 5, b: 12, c: 3, op: Operation.add, form: TaskForm.placeValue);
  ...
  test('two tasks with different hundreds are different tasks', () {
    const first =
        Task(a: 1, b: 2, c: 3, op: Operation.add, form: TaskForm.placeValue);
    const second =
        Task(a: 1, b: 2, c: 4, op: Operation.add, form: TaskForm.placeValue);
